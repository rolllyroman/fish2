...
#++++++++++++++++++++ 响应消息回调 ++++++++++++++++++++
    ...
    def onJoinGame(self, player, resp, isSendMsg):
        ...
        # 此处进行机器人管理操作
        if not player.isAI:
            print '非机器人玩家进入房间，机器人准备'
            player.server.robot_manager.manage(self, player)
    ...
    def onExitGame(self, player, sendMessage = True, byPlayer = False, isEndGame = False):
        ...
        self.redis.set('fish:%s:player:%s:houseNo'%(self.server.ID,player.uid),self.roomId)
        # flag
        print 'fish:%s:player:%s:houseNo'%(self.server.ID,player.uid)
        print self.roomId
        print '======================'
        self.redis.expire('fish:%s:player:%s:houseNo'%(self.server.ID,player.uid),500)

        print '玩家%s退出游戏，当前房间号%s,玩家位置%s'%(player.uid,self.roomId,player.chair)

        # 大厅玩家房间人数减一 为0时移除销毁房间info
        player_num = self.redis.hincrby('fish:%s:house:%s:info'%(self.server.ID,self.roomId),'player_num',-1)
        if int(player_num or 0) < 1:
            self.redis.delete('fish:%s:house:%s:info'%(self.server.ID,self.roomId),'player_num')
            self.redis.srem('fish:%s:house:set'%self.server.ID,self.roomId)
...
#++++++++++++++++++++ 通用房间服务工具 ++++++++++++++++++++
    ...
    def getOnlinePlayers(self, excludePlayers=()):
        return [player for player in self.players if (player and player not in excludePlayers and player.chair not in self.exitPlayers and not player.isAI)]
