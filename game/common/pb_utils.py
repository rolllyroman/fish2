...
from logic.global_control import  COIN_GUN_LEVELS,WARHEAD_GUN_LEVELS
...
def pbPlayerInfo(resp, game, side, isNeedMyData = False):
    ...
    resp.id = int(player.uid)
    if game.server.field_type == '1':
        levels_ = WARHEAD_GUN_LEVELS
    else:
        # levels_ =  COIN_GUN_LEVELS
        levels_ = game.server.globalCtrl.get_coin_gun_levels(player.vip_level)
    # resp.gunLevel = formatCoin(player.gunLevel)
    # last_value = int(redis.hget('users:%s'%player.uid,field_) or 0)
    try:
        cur =  levels_.index(player.gunCoin)
