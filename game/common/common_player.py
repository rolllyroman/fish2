    ...
    def loadDB(self, playerTable, isInit=True, account = None):
        ...
        if isInit:
            ...
            self.account, self.passwd, self.nickname, self.money, self.parentAg, self.currency,\
                    ...
                    self.totalProfitCoin4Ticket, self.cannonSkin, self.warhead,self.warhead_bonus,self.vip_level = redis.hmget(playerTable,\
                    ('account', 'password', 'nickname', 'money', 'parentAg',\
                    'currency', 'valid', 'sex', 'headImgUrl', 'maxScore', 'coin', 'exchange_ticket', 'ticket_profit', 'cannonSkin','warhead','warhead_bonus','vip_level'))

            self.warhead_bonus,self.warhead = int(self.warhead_bonus or 0),int(self.warhead or 0)
            self.vip_level = int(self.vip_level or 0)

            self.coin = int(self.coin or 0)
            try:
            ...
            try:
                ...
                self.totalProfitCoin4Ticket = 0
            self.coin, self.money = int(self.coin), round(float(self.money or 0), 2)

            # 如果是弹头场
            if self.server.field_type == "1":
                self.coin = self.warhead

            self.initCoin = self.coin
            # 记录最后一次未产生变化时弹头的数量
            self.pre_warhead = self.warhead
            ...
            #self.coin += coin
            # self.addCoin += coin
            redis.hincrby(playerTable, 'coin', -coin)
            if self.game:
