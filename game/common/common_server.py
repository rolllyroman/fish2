...
class CommonServer(Server):
    def __init__(self, *args, **kwargs):
        ...
        # 机器人管理者
        self.robot_manager = Robot_manager(self)
    ...
    def loadOtherChannalData(self, redis, globalCtrl):
        #房间配置
        room_name, base_coin, max_base_coin, step_base_coin, fish_need_count, fish_limit_count, max_add, min_add,self.field_type =\
                redis.hmget(FISH_ROOM_TABLE%(self.ID),\
                ('room_name', 'base_coin', 'max_base_coin', 'step_base_coin', 'fish_need_count', 'fish_limit_count', 'max_add', 'min_add','field_type'))
    ...
    def registerProtocolResolver(self):
        ...
        self.recverMgr.registerCommands( (\
            ...
            unpacker(pickfish_pb2.C_S_ROBOT_HIT_FISH, pickfish_pb2.C_S_RobotHitFish, self.robotHitFish), \
        ...
        self.senderMgr.registerCommands( (\
            ...
            packer(pickfish_pb2.S_C_GOLD_GAIN_WARHEAD, pickfish_pb2.S_C_Gold_gain_warhead), \
            ) )
...
#                return
    ...
    def onTryRegSucceed(self, player, cache, isReg, session, roomSetting, mode, isSendMsg = True):
        ...
        resp.field_type = self.field_type or '0'
        resp.result = True
    ...
    def tryJoinGame(self, player, resp, isSendMsg):
        ...
        redis = self.getPublicRedis()
        house_no = redis.get('fish:%s:player:%s:houseNo'%(self.ID,player.uid))
        print '======================================'
        print '======================================'
        print 'fish:%s:player:%s:houseNo'%(self.ID,player.uid)
        print house_no
        print '======================================'
        print '======================================'
        # 有指定房间的情况
        if house_no:
            games = self.globalCtrl.gameList
            # 指定房间存在
            print '------------------------'
            print games
            for g in games:
                print g.roomId
                print type(g.roomId)
                if g.roomId == str(house_no):
                    print '指定房间存在'
                    _game = g
                    redis.hincrby('fish:%s:house:%s:info' %(self.ID,_game.roomId), 'player_num', 1)
                    break
            # 指定房间不存在 创建新该房间
            else:
                print '指定房间不存在'
                _game = self.getGameModule(0, self.globalCtrl.maxPlayerCount, self)
                self.globalCtrl.addGame(_game, self.ID,house_no)
        ...
        # 快速加入房间
            ...
            print '随机加入房间中，游戏当前房间总数%s,空闲房间总数%s'%(len(self.globalCtrl.gameList),len(games))
            for g in self.globalCtrl.gameList:
                print '房间%s人员：%s'%(g.roomId,g.players)
            # 有空闲房间的游戏
            if games:
                _game = random.choice(games)
                redis.sadd('fish:%s:house:set'%self.ID, _game.roomId)
                redis.hincrby('fish:%s:house:%s:info' %(self.ID,_game.roomId), 'player_num', 1)
            # 没有空闲房间
            else:
                _game = self.getGameModule(0, self.globalCtrl.maxPlayerCount, self)
        ...
        '''
        # 原版本账号找到对应房间
        elif player.account in self.account2room and self.account2room[player.account][0] in self.globalCtrl.num2game:
            _game = self.globalCtrl.num2game[self.account2room[player.account][0]]
        '''
        redis.delete('fish:%s:player:%s:houseNo'%(self.ID,player.uid))
        _game.onJoinGame(player, resp, isSendMsg)

    def tryJoinGame_0(self, player, resp, isSendMsg):
        if player.game:
            log(u'[try join game][error]account[%s] already in game.'%(player.account))
            return
    ...
    def robotHitFish(self, player, req):

        if not player.game:
            log(u'[<<<robot>>> hit fish][error] account[%s] is not in game.'%(player.account))
            return

        robot = self.robot_manager.uid_to_robot.get(str(req.uid))
        if robot:
            robot.hitFish(req.bulletId,req.fishIds,req.timestamp)
    ...
    def onFishLogout(self, pipe, player, dateTimeStr, redis): #退出捕鱼时需要记录的数据
        ...
        #  弹头场
        if player.server.field_type == "1":
            pipe.hmset(player.table,{
                "warhead":int(player.coin),
            })
        else:
            # 金币场
            pipe.hmset(player.table,{
                "warhead":player.warhead,
                "warhead_bonus":player.warhead_bonus,
...
#            self.currencyAgentCashRefreshTimestamp = timestamp
    ...
    def onServiceMemberRefresh(self, timestamp, memberAccount):
        print '=================================================================='
        print 'onServiceMemberRefresh(self, timestamp, memberAccount)'
        print "memberAccount:%s" %memberAccount
        print "self.account2players:%s"%self.account2players
        print '=================================================================='
