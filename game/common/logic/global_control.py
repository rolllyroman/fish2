...
COIN_GUN_LEVELS = [50,60,70,80,90,100,200,300]
WARHEAD_GUN_LEVELS = [100,200,300,400,500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 10000, 20000, 30000, 40000, 50000, 100000, 200000]
...
class GlobalControl(GameObject):
    ...
    def get_coin_gun_levels(self,vip_level):
        add = [400,500,1000,2000,3000,4000,5000,6000,7000,8000,9000,10000]
        levels_ = COIN_GUN_LEVELS
        if vip_level >= 1:
            levels_ = COIN_GUN_LEVELS + add[:4]
        if vip_level >= 2:
            levels_ = COIN_GUN_LEVELS + add[:7]
        if vip_level >= 3:
            levels_ = COIN_GUN_LEVELS + add
        return levels_
    ...
    def addGame_0(self, game, gameId):
    ...
    def addGame(self, game, gameId,house_no):
        ...
        # 指定玩家加入某个房间 flag
        if house_no:
            game.roomId = house_no
        else:
            game.roomId = self.getRoomNum()

        redis.sadd('fish:%s:house:set'%self.sever.ID,game.roomId)
        redis.hset('fish:%s:house:%s:info'%(self.sever.ID,game.roomId),'player_num',1)
    ...
    def upGunLevelNCoin(self, player):
        """
        升级处理
        """
        if player.server.field_type == '1':
            field_ = 'last_warhead_gun_value'
            levels_ = WARHEAD_GUN_LEVELS
        else:
            field_ = 'last_coin_gun_value'
            levels_ = self.get_coin_gun_levels(player.vip_level)

        try:
            index = levels_.index(player.gunCoin)
        except:
            index = 0

        if index < len(levels_)-1:
            cur = index+1
        else:
            cur = 0

        if cur > len(levels_)*2/3:
            gl = 3
        elif cur > len(levels_)/3:
            gl = 2
        else:
            gl = 1

        player.gunLevel = cur
        player.gunCoin = levels_[cur]
        player.redis.hset('users:%s'%player.uid,field_,player.gunCoin)

        return gl,player.gunCoin
        ...
        #金币超出上限升级，否则只是加金币
        print '--------------------------------------------------'
        print 'self.gunLevels:%s,        gunInfo:%s,         gunInfo.coinRange:%s' %(self.gunLevels,self.gunLevels[level],gunInfo.coinRange)
        print '--------------------------------------------------'
    ...
    def deGunLevelNCoin(self, player):
        """
        降级处理
        """
        if player.server.field_type == '1':
        ...
        if cur < len(levels_)/3:
            gl = 1
        elif cur < len(levels_)*2/3:
            gl = 2
        else:
            gl = 3
