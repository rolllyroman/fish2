...
class Player(CommonPlayer):
    def __init__(self):
        ...
        self.addChance = 1.2 # 增加多少
        # self.downChance = 0.8 # 减少多少
    ...
    def upChance(self):
        ...
        self.chanceInterface = round(float(redis.get('fish:up:chance') or 1.2),2)
        print(u"当前捕获*%s" % self.chanceInterface)

    def downChance(self):
        "降低机率"
        # print '------------------down chance-----------------------'
        redis = self.factory.getPublicRedis()
        self.chanceInterface = round(float(redis.get('fish:down:chance') or 0.8),2)
        print(u"当前捕获*%s" % self.chanceInterface)
    ...
    def clearChance(self):
        ...
        self.chanceInterface = self.chanceDefault
        print(u"当前捕获*%s" % self.chanceInterface)
    ...
    def loadDB(self, playerTable, isInit=True, account = None):
        ...
        # self.gunCoin = int(self.redis.hget('fish:channel:%s:info'%self.factory.ID,'base_coin' or 10))
        if self.server.field_type == '1':
            field_ = 'last_warhead_gun_value'
            levels_ = WARHEAD_GUN_LEVELS
        else:
            field_ = 'last_coin_gun_value'
            levels_ = self.server.globalCtrl.get_coin_gun_levels(self.vip_level)
        self.gunCoin = int(redis.hget('users:%s'%self.uid,field_) or levels_[0])
