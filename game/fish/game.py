...
class Game(CommonGame):
    def __init__(self, desk, maxPlayerPerDesk, server):
        ...
        #self.fishMgr = FishMgr(self,random.randint(self.server.globalCtrl.addFishMinCount, self.server.globalCtrl.addFishMaxCount))
        ...
        self.redis = self.server.getPublicRedis()
...
#            return None
    ...
    def onHitFish(self, player, timestamp, bulletId, fishIds):
        print "======================================================="
        print "on hit fish:          player.coin:%s"%player.coin
        print "======================================================="

        '''
        #  非弹头场获得弹头
        w_bonus = int(player.warhead_bonus or 0)
        if self.server.field_type != '1' and w_bonus > 0:
            ran_res = random.randint(0,3)#w_bonus if w_bonus > 1 else 1)
            add_warhead = int(ran_res if ran_res > 0 else 1)
            # 玩家获取掉落弹头
            player.warhead += add_warhead
            player.warhead_bonus -= add_warhead
            # 统计掉落弹头
            self.redis.incr("drop:warhead:sum", add_warhead)
            # 记录玩家获得弹头
            war_key = str(datetime.now())[:10]
            self.redis.hincrby('user:%s:gain:warhead:num'%player.uid,war_key,add_warhead)
            rep = pickfish_pb2.S_C_Gold_gain_warhead()
            rep.add_warhead = add_warhead
            rep.warhead = player.warhead
            rep.side= player.chair
            self.sendAll(rep)

        '''
        # 在此统计金币或弹头的投入产出情况
        # 记录此刻的金币弹头数量
        player.pre_coin = player.coin
        ...
        cost_table = 'user:%s:cost:warheads:date:%s'%(player.uid,date_)
        pipe.incrby(cost_table,bullet.realCoin)
        pipe.expire(cost_table,3600*24*15)
        pipe.execute()
...
#                fish.outTimestamp = fish.outTimestamp + FREEZE_TICK*bullet.decelerate
        ...
        if pickProto.fishs:
            if addCoin > 0:
                for pf in pickProto.fishs:
                    ...
                    self.statis_today_gain(player,pf.gainCoin,bullet)
                    if fcoin > taxCoin:
        ...
        self.statis_coin_or_warhead(player)

    def statis_today_gain(self,player,gainCoin,bullet):

        uid = player.uid
        pipe = self.redis.pipeline()
        date_ = str(datetime.now())[:10]
        fish_num_table = 'user:%s:gain:fish:date:%s'%(uid,date_)
        if self.server.field_type != '1':
            table = 'user:%s:gain:coins:date:%s'%(uid,date_)
        else:
            table = 'user:%s:gain:warheads:date:%s'%(uid,date_)

        pipe.incrby(table,gainCoin)
        pipe.incrby(fish_num_table,1)
        pipe.expire(table,3600*24*15)
        pipe.expire(fish_num_table,3600*24*15)
        pipe.execute()

    def statis_coin_or_warhead(self,player):
        # 金币场
        pipe = self.redis.pipeline()
        if self.server.field_type != '1':
            # 玩家得利,统计产出金币
            produce_table = "produce:coin:sum"
            earnings_table = "earnings:coin:sum"
            self.redis.sadd('fish:coin:player:set',player.uid)
            user_cost_sum = 'user:%s:cost:sum'%player.uid
            user_gain_sum = 'user:%s:gain:sum'%player.uid
            user_play_in = 'user:%s:play:in'%player.uid
            user_play_out = 'user:%s:play:out'%player.uid
            # player_get_today = 'user:%s:get:warheads:date:%s'%(player.uid,str(datetime.now())[:10])
        # 弹头场
        else:
            self.redis.sadd('fish:test:player:set',player.uid)
            produce_table = "produce:warhead:sum"
            earnings_table = "earnings:warhead:sum"
            user_cost_sum = 'test:%s:cost:sum'%player.uid
            user_gain_sum = 'test:%s:gain:sum'%player.uid
            user_play_in = 'test:%s:play:in'%player.uid
            user_play_out = 'test:%s:play:out'%player.uid
            # player_get_today = 'user:%s:get:coins:date:%s'%(player.uid,str(datetime.now())[:10])
        # 弹头场

        produce = int(player.coin - player.pre_coin)
        date_key = str(datetime.now())[:10]
        if produce > 0:
            pipe.incrby(produce_table,produce)
            pipe.incrby(user_gain_sum,produce)
            pipe.incrby(user_play_out,produce)
        elif produce < 0:
            earnings = -produce
            pipe.incrby(earnings_table,earnings)
            pipe.incrby(user_cost_sum,earnings)
            pipe.incrby(user_play_in,earnings)
    ...
    def onSwitchGun(self, player, upgrade):
        ...
        if upgrade:
            ...
            player.gunLevel, player.gunCoin = self.server.globalCtrl.upGunLevelNCoin(player)
        else:
            player.gunLevel, player.gunCoin = self.server.globalCtrl.deGunLevelNCoin(player)
    ...
    def onSwitchGun_0(self, player, upgrade):
        if player.isInGoldTime():
            log(u'[try switch gun][error]player[%s] is in gold time.'%(player.nickname), LOG_LEVEL_RELEASE)
            return
