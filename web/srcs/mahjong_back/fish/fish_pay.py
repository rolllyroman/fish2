...
def do_buyCoin(redis,session):
    ...
    check_code,check_msg,user_table,osid = check_session_verfiy(redis,'/fish/buyCoin/',SessionTable,account,sid,verfiySid)
    ...
    coin = redis.hincrby(user_table,'coin',cards)

    warhead_bonus = int(int(cards)/10)


    wh = redis.hincrby(user_table,'warhead_bonus',bullet_head)

    print '-' *66
    print '-' *66
    print 'uid:'
    print user_table.split(':')[1]
    print 'coin:'
    print res_coin 
    print 'res_warhead_bonus:'
    print wh
    print '-' *66
    print '-' *66
...
def do_onWeChatPay(redis, session):
    ...
    SessionTable,account,uid,verfiySid = getInfoBySid(redis,sid)
    check_code,check_msg,user_table,osid = check_session_verfiy(redis,'/fish/checkOrder/',SessionTable,account,sid,verfiySid)
    ...
    signDict = {
        ...
        'spbill_create_ip'  :       ip,
        'notify_url'        :       NOTIFY_URL_FISH%('fishwxpay.dongshenggame.cn'),
...
def do_onWeChatPay4TX(redis, session):
    ...
    check_code,check_msg,user_table,osid = check_session_verfiy(redis,'/fish/checkOrder/',SessionTable,account,sid,verfiySid)
    log_util.debug('[try checkOrder] check_code[%s] check_msg[%s]'%(check_code,check_msg))
    print '[try checkOrder] check_code[%s] check_msg[%s]'%(check_code,check_msg)
    if int(check_code)<0:
        if check_code == -4:
            return {'code':check_code,'msg':check_msg,'osid':osid}
        return {'code':check_code,'msg':check_msg}
    ...
    log_util.debug('[try goods] player cards[%s] goodsPrice[%s].'%(cards,goodsPrice))
    print '[try goods] player cards[%s] goodsPrice[%s].'%(cards,goodsPrice)
